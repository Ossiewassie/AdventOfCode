...
function calculatePath(newroute) {
    ...
    newroute.forEach(element => {
        ...
        distance = parseInt(element.split(/U|R|D|L/)[1]);
        ...
        switch (direction) {
            ...
            case 'L':
                ...
                break;
            default:
                error.log('Yoooo this shit broke lmao');
                return;
        }

        path.push([posXY[0],posXY[1]]);
    });
    
    return path;
}

console.log(calculatePath(route1));

function findIntersect(r1,r2) {
    r1.forEach(element1 => {
        r2.forEach(element2 => {
            banan = '1';
            banan += `compare ${element1[0]} to ${element2[0]}`;
            fs.writeFileSync('poo.txt', banan);
        });        
    });
}

findIntersect(calculatePath(route1),calculatePath(route2));